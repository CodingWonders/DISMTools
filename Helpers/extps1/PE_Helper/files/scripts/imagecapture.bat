...
IF %_DEBUG% EQU 1 echo Removing rollback BCD entry...
if exist "%SYSTEMDRIVE%\SysprepPrepTool\rollback_env_entry_guid.txt" (
	for /f %%a in ('type "%SYSTEMDRIVE%\SysprepPrepTool\rollback_env_entry_guid.txt"') do bcdedit /delete %%a /f
)
