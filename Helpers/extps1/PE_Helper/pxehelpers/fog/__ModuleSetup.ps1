...
Invoke-WebRequest "$downloadUri" -OutFile "$env:TEMP\fogapi.zip"
if ($? -eq $false) {
    Write-Host "Could not download release file."
    return
}

if (Test-Path "$env:TEMP\fogapi.zip" -PathType Leaf) {
    Write-Host "Release has been downloaded. Extracting it..."
    Expand-Archive -Path "$env:TEMP\fogapi.zip" -Destination "$PSScriptRoot" -Force
    if ($?) {
        New-Item -Path "fogready" -Force | Out-Null
        $assetData.tag_name | Out-File "fogready" -Force
    }
    Remove-Item "$env:TEMP\fogapi.zip" -Force
}
